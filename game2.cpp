...
//расстановка фишек на доске; их начальное положение
...
void Game_Wolves_rabbit:: playWithThemselves(int & i, int & j, int & flagPrintMap)
{
	reset(); // расставить фишки на шахматной доске
	//prepareMap();
	int maxRecursivLevelRabbit = i;//recursivLevelRabbit;
	int maxRecursivLevelWolves = j;//recursivLevelWolves;
	//int n = numberExperiments;
	while( !isGameOver() )
	{
		runMinMax(MT_Rabbit, 0, maxRecursivLevelRabbit, INT_MIN, INT_MAX);
		//prepareMap();
		UpdatePrintMap();
		isGameOver();
		//printMap();
		
		runMinMax(MT_Wolf, 0, maxRecursivLevelWolves, INT_MIN, INT_MAX);
...
int Game_Wolves_rabbit:: runMinMax(MonsterType monster, int recursivLevel, int maxRecursivLevel, int alpha, int beta)
{
	if (recursivLevel == 0)
		prepareMap();

	if (recursivLevel >= maxRecursivLevel)
	{
	...
	for(int i = (isWolf ? 0 : 8); i < (isWolf ? 8 : 12); i++)
	{
		...
		{ 
			...
			test = runMinMax(isWolf ? MT_Rabbit : MT_Wolf, recursivLevel + 1, maxRecursivLevel, alpha, beta);
			...
			{
				if( test == MinMax) //для одинаковых оценок случайный выбор
				{
					srand(time(NULL));
					int rnd = rand() % 2;
					if(rnd == 0) {
